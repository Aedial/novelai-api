...
async def main():
    parser = ArgumentParser(description = "Delete the stories matching all the selected filters, "
                                          "and save them in <backup_directory>. Filters that are not set are ignored")
    parser.add_argument("backup_directory", nargs = "?", default = ".",
                        help = "Directory to save the deleted stories to")

    parser.add_argument("--created-before", type = int, required = False,
                        help = "Select all items created strictly before this time (in ms since Epoch)")
    parser.add_argument("--created-after", type = int, required = False,
                        help = "Select all items created strictly after this time (in ms since Epoch)")
    parser.add_argument("--created-between", type = int, required = False, nargs = 2, default = [], action = "append",
                        help = "Select all items created strictly between these times (in ms since Epoch)")

    parser.add_argument("--last-updated-before", type = int, required = False,
                        help = "Select all items last updated strictly before this time (in ms since Epoch)")
    parser.add_argument("--last-updated-after", type = int, required = False,
                        help = "Select all items last updated strictly after this time (in ms since Epoch)")
    parser.add_argument("--last-updated-between", type = int, required = False, nargs = 2, default = [],
                        action = "append", help = "Select all items last updated strictly between these times "
                                                  "(in ms since Epoch)")
    ...
    parser.add_argument("--has-memory", type = str_to_bool, required = False,
                        help = "Select all items having text in Memory or not")
    parser.add_argument("--string-in-memory", type = str, required = False, default = [], nargs = '+',
                        action = "append",
                        help = "Select all items having the specified text in Memory. "
                               "Text in the same argument are AND, text on a different argument are OR.")

    parser.add_argument("--has-an", type = str_to_bool, required = False,
                        help = "Select all items having text in A/N or not")
    parser.add_argument("--string-in-an", type = str, required = False, default = [], nargs = '+', action = "append",
                        help = "Select all items having the specified text in A/N. "
                               "Text in the same argument are AND, text on a different argument are OR.")

    parser.add_argument("--has-lorebook", type = str_to_bool, required = False,
                        help = "Select all items having a Lorebook or not")
    parser.add_argument("--key-in-lorebook", type = str, required = False, default = [], nargs = '+', action = "append",
                        help = "Select all items having the specified keys in Lorebook. "
                               "Keys in the same argument are AND, keys on a different argument are OR.")
